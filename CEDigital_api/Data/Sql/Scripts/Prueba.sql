-- Poblaci�n para Carrera
INSERT INTO Carrera (codigo_carrera, nombre) VALUES
('CE', 'Escuela de Ingenier�a en Computadores'),
('EE', 'Escuela de Ingenier�a en Electr�nica'),
('CI', 'Escuela de Ciencias del Lenguaje'),
('CS', 'Escuela de Ciencias Sociales'),
('MA', 'Escuela de Matem�tica'),
('QU', 'Escuela de Qu�mica');


-- Poblaci�n para Curso
INSERT INTO Curso (codigo_curso, nombre, creditos, codigo_carrera) VALUES
('CI0205', 'Prueba avanzada ingl�s', 0, 'CI'),
('MA0101', 'Matem�tica general', 2, 'MA'),
('CE1101', 'Introducci�n a la programaci�n', 3, 'CE'),
('CE1104', 'Fundamentos de sistemas computacionales', 3, 'CE'),
('MA1102', 'C�lculo diferencial e integral', 4, 'MA'),
('MA1403', 'Matem�tica discreta', 4, 'MA'),
('QU1102', 'Laboratorio de qu�mica b�sica i', 1, 'QU'),
('QU1106', 'Qu�mica b�sica i', 3, 'QU');

-- Poblaci�n para Semestre
INSERT INTO Semestre (anio, periodo) VALUES
(2025, '1');

-- Poblaci�n para Grupo
...
-- 2. Obtener ids de los rubros autom�ticos
DECLARE @id_rubro_quices INT = (SELECT id_rubro FROM Rubro WHERE nombre = 'Quices' AND id_grupo = @id_grupo);
DECLARE @id_rubro_examenes INT = (SELECT id_rubro FROM Rubro WHERE nombre = 'Ex�menes' AND id_grupo = @id_grupo);
